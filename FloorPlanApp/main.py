...
if platform == "android":
    ...
    try:
        ...
        # Optional: Check if a permission is granted
        def has_storage_permission():
            return check_permission(Permission.WRITE_EXTERNAL_STORAGE)


        # Get primary external storage path (e.g., /storage/emulated/0)
        storage_path = primary_external_storage_path()
        print(f"External storage path: {storage_path}")

        # You can now use this path to save files, photos, etc.
        photo_dir = f"{storage_path}/Pictures/MyApp"

    except ImportError as e:
        print(f"Android module import error: {e}")

    # Schedule permission request (if using Kivy)
    from kivy.clock import mainthread


    @mainthread
    def request_permissions_on_main():
        on_start()


    request_permissions_on_main()

else:
    print("Running on non-Android platform (e.g., Windows, macOS, Linux)")
    # Define fallback behavior
    storage_path = "."


    def has_storage_permission():
        return True  # Assume allowed on desktop
